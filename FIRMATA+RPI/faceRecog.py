...
class facialDatector():
    ...
    def __init__(self,width,height,model_path=r"C:\Users\zanyi\Documents\GitHub\AIMV\faceDetection.pt", yPercentage=10, xPercentage=15):
    ...
    def getVal(self,frame):
        ...
        if self.prediction[0]:
            ...
            #mark edge boxes
            cv2.rectangle(self.frame,(self.bottomHeight,self.topHeight),(self.topWidth,self.bottomWidth),color=(255,0,0),thickness=1) 
            ...
            #x left/right limit
            self.frame = cv2.line(self.frame, (self.limitLeft,0), (self.limitLeft,round(self.height)), (0,255,0), 1)
            self.frame = cv2.line(self.frame, (self.limitRight,0), (self.limitRight,round(self.height)), (0,255,0), 1)
            #midpoint & confidence
            ...
            self.servoYlimit=90
            self.exists=True
            return [self.exists,self.frame,[self.limitBottom,self.limitTop, self.limitLeft, self.limitRight],[self.midpoint[0],self.midpoint[1]]] 
        else:
